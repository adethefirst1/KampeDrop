...
import {
  listRiderPayoutBanks,
  maskRiderAccountNumber,
  resolveRiderBankAccount,
  saveRiderBankDetails,
  type PaystackBankOption,
  type RiderBankDetails,
} from '../../lib/ridersApi'
...
export function RiderBankDetailsSection({ accessToken, bank, onSaved }: Props) {
  const hasBank =
    Boolean(bank.accountNumber && bank.accountName && bank.bankCode) ||
    bank.hasRecipient
  ...
  const loadBanks = useCallback(async () => {
    ...
    setBanksError(null)
    const result = await listRiderPayoutBanks(accessToken)
  ...
  function resetForm(prefill?: RiderBankDetails) {
  ...
  async function onVerify(e: FormEvent) {
    ...
    setError(null)
    setBusy(true)
    const result = await resolveRiderBankAccount(accessToken, {
      bankCode,
      accountNumber,
    })
    setBusy(false)
  ...
  async function onSave() {
    ...
    setError(null)
    setBusy(true)
    const result = await saveRiderBankDetails(accessToken, {
      bankCode,
        ...
        <dl className="mt-4 space-y-2.5">
          ...
          <div>
            ...
            <dd className="mt-0.5 font-mono text-sm font-bold tabular-nums text-ink">
              {maskRiderAccountNumber(bank.accountNumber)}
            </dd>
          </div>
          <div>
            <dt className="text-[10px] font-bold uppercase tracking-[0.12em] text-muted">
      ...
      {step === 'form' ? (
        <form onSubmit={onVerify} className="mt-3 space-y-3">
          ...
          <button
            ...
            disabled={
              busy || banksLoading || !bankCode || accountNumber.length !== 10
            }
          >
        ...
        <div className="mt-3 space-y-3">
          <div className="rounded-2xl bg-lagoon/10 px-3.5 py-3 ring-1 ring-lagoon/20">
            ...
            <p className="mt-2 text-xs font-semibold text-muted">
              Confirm this is your account before saving. If the name is wrong,
