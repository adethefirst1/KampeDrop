/**
 * Rider bank — list NGN banks + resolve account number (Paystack).
 *
 * Auth: rider access_token in body (same credential as portal RPCs).
 * Kept separate from vendor-resolve-account — never share party credentials.
 *
 ...
 * Saving is rider-save-bank after the rider confirms the name.
 */
...
Deno.serve(async (req) => {
  ...
  const rider = await riderFromToken(admin, token)
  if (!rider) {
